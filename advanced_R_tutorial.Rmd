...
We, data scientists, deal with data. We analyze data in an attempt to get insightful information about the data first. We then try to extract useful information from the data so that a sensible decision may/can be made. What data to collect and what we are looking for from the data are the important steps toward a successful study. A doctor without years training both in gathering knowledge in medicine and clinical practice with patients can not be called a doctor not to mention being a good doctor. Same here, we need to accumulate experience dealing with data. Starting with a set of R functions focusing on manipulating, summarizing and visualizing data we start our journey towards a great data scientist. 
...
In addition to `tidyverse` and `data.table`, packages `skimr`, `nycflights13`, `gapminder`, `ggthemes`, `ggpubr` and `plotly` will be used in the tutorial. We use `p_load` function of `pacman`, to load all the packages at once. It will first check whether packages are installed. If not, it will install the yet to be installed automatically and then load all the packages.
...
Before we dig into details about `dplyr`, let us first find out information about `dplyr` using function
`browseVigenttes("package name")` 

```{r}
browseVignettes("dplyr")
```
Depending on the package we will be able to see documents about it, including the source file, Rcode etc. For `dplyr` we may explore *Introduciton to `dplyr`*
...
To illustrate these commands, we will use a dataset called `flights` in `nycflights13` package that contains 336,776 flights departed from New York City in 2013. `dplyr` allows you to gather insight from a dataset without altering the original dataset. 
It is considered best practice not to alter the original dataset. For example in this case, we will never overwrite the existing dataset 'flights'. We will first take a look at the the data set and summary statistics.
...
summary(flights)
```
Notice that there are many missing values in the data set. This raises alerts to us about the data. 

We could use a package `skimr` to get a better organized summary. 
```{r, summary of flights, results="hide"}
# summary(flights)
skim(flights)
```
...
`n_distinct(dest)` returns the number of distinct destinations. From this table we can see that EWR has flights to the largest number of destinations (56). We can also see LGA flights has a lower average distance than those of EWR & JFK.  
...
Another example is to find out some worst flights of AA and UA.
```{r}
flights %>%
  filter(carrier %in% c("AA", "UA")) %>%
...
United Airline, American Airline and Lufthansa. We compare the join results using
...
`ggplot` is THE tool for data exploratory analysis.
Data exploratory analysis is a way to develop an understanding of your data and
we cannot stress more about understanding your data as data scientists. 
`ggplot` is one of the best tools for plotting graphs among other softwares/libraries
...
Functional programming language write programs by applying and composing functions.
...
Remeber `.SDcols` and `.SD` (Subset of Data.table) are for selecting columns.
```{r}
...
or using `by.x` to indicates the key variable for `x` table and `by.y` for `y` table.
It uses argument
...
Lastly, we introduce a way to run Rstudio in the cloud based on [Rstudio Server](https://support.rstudio.com/hc/en-us/articles/234653607-Getting-Started-with-RStudio-Server). This can come in handy later on in the class when we do more complex analysis with larger datasets, as it will greatly speed up processing times. 
